  ...
  <div class="space-y-6">
    <section class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
      <div>
        ...
        <h1 class="mt-1 text-3xl font-black tracking-tight text-slate-950">Lowongan</h1>
        <p class="mt-1 text-sm text-slate-500">Kelola seluruh data dan status lowongan rekrutmen.</p>
      </div>
    </section>
    ...
    <div v-if="actionError" class="rounded-2xl border border-rose-200 bg-rose-50 px-5 py-4 text-sm font-semibold text-rose-700">
      ...
      <section class="dashboard-card space-y-5">
        ...
        <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div class="grid gap-3 sm:grid-cols-[104px_156px_140px_minmax(220px,1fr)_auto_auto] lg:flex-1">
            ...
            <table class="w-full min-w-[1050px] table-fixed divide-y divide-slate-200 text-left text-sm">
              ...
              <thead class="bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
                <tr>
                  ...
                  <th class="px-4 py-3 text-center">Buka</th>
                  <th class="px-4 py-3 text-center">Tutup</th>
                ...
                <tr v-for="lowongan in lowonganList" :key="lowongan.id" class="hover:bg-indigo-50/40">
                  ...
                  <td class="px-4 py-4">
                    ...
                    <p
                      ...
                      :title="lowongan.unit || 'Unit belum ditentukan'"
                    >
                      {{ lowongan.unit || 'Unit belum ditentukan' }}
                    </p>
                  </td>
                  <td class="whitespace-nowrap px-4 py-4 text-center text-slate-600">{{ formatDate(lowongan.tanggalBuka) }}</td>
                  <td class="whitespace-nowrap px-4 py-4 text-center text-slate-600">{{ formatDate(lowongan.tanggalTutup) }}</td>
                  ...
                  <td class="px-4 py-4">
                    <div class="flex items-center justify-center gap-1.5">
                      <button
                        type="button"
                        class="muted-button grid h-9 w-9 place-items-center p-0 text-slate-500 hover:border-indigo-200 hover:bg-indigo-50 hover:text-indigo-600"
                        title="Lihat detail"
                        aria-label="Lihat detail lowongan"
                        @click="showDetail(lowongan.id)"
                      >
                      ...
                      <button
                        type="button"
                        class="muted-button grid h-9 w-9 place-items-center p-0 text-slate-500 hover:border-amber-200 hover:bg-amber-50 hover:text-amber-600"
                        title="Edit lowongan"
                        aria-label="Edit lowongan"
                        @click="editLowongan(lowongan)"
                      >
                        <svg class="h-4.5 w-4.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
                          <path d="M12 20h9" />
                      ...
                      <button
                        type="button"
                        class="grid h-9 w-9 place-items-center rounded-xl border border-rose-100 bg-white p-0 text-rose-500 shadow-sm transition hover:bg-rose-50 hover:text-rose-700"
                        title="Hapus lowongan"
                        aria-label="Hapus lowongan"
                        @click="deleteLowongan(lowongan.id)"
                      >
                        <svg class="h-4.5 w-4.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
                          <path d="M3 6h18" />
                          <path d="M8 6V4h8v2" />
